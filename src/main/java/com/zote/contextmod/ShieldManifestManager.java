...
public class ShieldManifestManager {
     ...
     *  Positions are recomputed from scratch every tick from set.shields.size() (Fibonacci
     *  sphere / focus grid), so shrinking the list mid-formation is safe — no per-shield
     *  index bookkeeping needed elsewhere. */
    public static String dismissSome(MinecraftServer server, String playerName, int count) {
        return runOnServer(server, player -> {
            UUID uuid = player.getUuid();
            ShieldSet set = playerShields.get(uuid);
            if (set == null || set.shields.isEmpty()) {
                return "{\"ok\":true,\"removed\":0,\"count\":0}";
            }
            ServerWorld world = player.getServerWorld();
            ...
            if (set.shields.isEmpty()) playerShields.remove(uuid);
            return "{\"ok\":true,\"removed\":" + toRemove + ",\"count\":" + set.shields.size() + "}";
        }, playerName);
    }
